...
description: "Use this agent when presented with a new feature to specify"
model: opus
color: blue
---
...
Create tickets as the user stories emerge, and link them to the high-level ticket. Use the Story type for user stories that create discrete user-facing value. Use the Task type to create tasks for developers, devops or PMs to complete.
...
9. For each of the Jira tickets you create, **delegate to the planning-agent**.
...
When delegating ticket planning to the `planning-agent`, provide the ticket number, as well as the location of the SPECIFICATION.md and the completed RESEARCH.md.

Example delegation:
```
Time to plan [jira-ticket-number]. Please read the ticket overview, the provided SPECIFICATION.md and RESEARCH.md, then complete your planning steps.
```